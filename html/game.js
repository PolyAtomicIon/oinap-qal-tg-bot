...
function preload() {
  ...
  game.load.bitmapFont(
    "heartbit-72",
    "assets/heartbit-72.png",
    "assets/heartbit-72.fnt"
  );
...
function create() {
  ...
  score_text = game.add.bitmapText(
    game.width * 0.75,
    game.height * 0.85,
    "heartbit",
    "Score: " + score,
...
function update() {
  if (!gameOver) {
    ...
    game.physics.arcade.overlap(
      ...
      defeat,
      function (e, p) {
        return !(e.aysncDestroy || p.asyncDestroy);
      },
      this
    );
  }
...
function defeat(enemy, projectile) {
  if (enemy == enemies[0]) {
    // Enemy is correct, increase difficulty and score
    score += 10;
    ...
    // Wrong enemy, do not remove the enemy and decrease heart count
    if (hearts.length > 0) {
      hearts.shift().destroy();
      if (hearts.length == 0) setGameOver();
    }
  }
...
function setGameOver() {
  ...
  if (uid && msgid && chatid) {
    console.log("TAKS");
    $.get(
      "https://biz-oinaimyz.herokuapp.com/setscore/uid/" +
        uid +
        "/chat/" +
        chatid +
        "/msg/" +
